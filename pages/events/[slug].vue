...
// Guardar ultimo evento visitado en localStorage (solo cliente)
if (process.client) {
  watch(event, (val) => {
    if (val) {
      try {
...
const selectedDate = computed<PublicEventDate | null>(() => {
...
// Estado para las cantidades seleccionadas por categoria
          ...
          <div class="d-flex flex-wrap justify-center align-center ga-2">
            ...
            >
              ...
              <span v-if="mainDate.ends_at">
                &nbsp;&ndash;&nbsp;{{ formatDateTime(mainDate.ends_at) }}
              </span>
            </span>
            <v-chip
              ...
              size="small"
            >
              + {{ additionalDatesCount }} fecha{{ additionalDatesCount > 1 ? 's' : '' }} mas
            </v-chip>
          </div>
          <p
            v-if="event.location"
                  ...
                  >
                    <div class="text-body-2">
                      ...
                      <span v-if="date.ends_at">
                        &nbsp;&ndash;&nbsp;{{ formatDateTime(date.ends_at) }}
                      </span>
                    </div>
              ...
              <v-card-text>
                <div v-if="!availableDates.length">
                  <p class="text-caption text-medium-emphasis mb-0">
                    Proximamente se anunciaran las localidades y precios.
                  </p>
                </div>
                <div v-else>
                  <v-select
